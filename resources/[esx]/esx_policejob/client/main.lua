	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
	{
		...
		if data.current.value == 'swat_wear' then
			if ESX.GetPlayerData().job.ext == 'swat' then
				...
				ESX.ShowNotification("~r~~h~Shoma SWAT nistid!")
			end
		end
...
function OpenVehicleSpawnerMenu(station, partNum)
	...
	for i=1, tonumber(PlayerData.job.grade) do
		if PlayerData.job.ext == 'swat' then
			if Config.AuthorizedVehicles[i].swat then
				for k,v in ipairs(Config.AuthorizedVehicles[i].swat) do
					table.insert(elements, { label = v.label, model = v.model})
				end
			end
		elseif PlayerData.job.ext == 'sheriff' then
			if Config.AuthorizedVehicles[i].sheriff then
				for k,v in ipairs(Config.AuthorizedVehicles[i].sheriff) do
					print(v.label)
					table.insert(elements, { label = v.label, model = v.model})
				end
			end
		end
		if Config.AuthorizedVehicles[i].shared then
			for k,v in ipairs(Config.AuthorizedVehicles[i].shared) do
				table.insert(elements, { label = v.label, model = v.model})
			end
		end
	end
...
function OpenIdentityCardMenu(player)
	...
	ESX.TriggerServerCallback('esx_policejob:getOtherPlayerDataCard', function(data)
...
AddEventHandler('esx_policejob:getarrested', function(playerheading, playercoords, playerlocation, faction)
	playerPed = GetPlayerPed(-1)
	SetCurrentPedWeapon(playerPed, GetHashKey('WEAPON_UNARMED'), true) -- unarm player
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(1, Keys['Y']) and IsControlPressed(1, Keys['LEFTSHIFT']) and PlayerData.job ~= nil and PlayerData.job.name == 'police' and PlayerData.job.grade > 0 then
	...
	if PlayerData.job ~= nil and PlayerData.job.name == 'police' and PlayerData.job.grade > 0 then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'police' and players[i].job.grade > 0 then
...
function SendPoliceDistressSignal()
	...
	if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
		...
		local plate = GetVehicleNumberPlateText(vehicle)
		TriggerServerEvent("esx_policejob:soundplay", "panic", 0.5, PedPosition.x, PedPosition.y, plate, nil, true, true)
	else
			ESX.TriggerServerCallback('esx_policejob:getIcName', function(name)
			TriggerServerEvent("esx_policejob:soundplay", "panic", 0.5, PedPosition.x, PedPosition.y, nil, name, true)
		end)
	end
...
function SendBackup()
	...
	ESX.ShowNotification(_U('distress_sent'))
	TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 5.0, 'demo', 1.0)

	if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
		local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1))
		local plate = GetVehicleNumberPlateText(vehicle)
		TriggerServerEvent("esx_policejob:soundplay", "demo", 0.5, PedPosition.x, PedPosition.y, plate, nil, false, false)
	else

		TriggerServerEvent("esx_policejob:soundplay", "demo", 0.5, PedPosition.x, PedPosition.y, nil, PlayerData.name, false)
...
Citizen.CreateThread(function()
	while true do
        ...
        Citizen.Wait(1)
        -- if IsControlPressed(2, Keys["M"]) and GetLastInputMethod(2) and Busy == false and PlayerData.job.name ~= nil and PlayerData.job.name == "police" then
		-- 		TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 5.0, 'on', 0.7)
		-- 		TriggerServerEvent('esx_policejob:playSoundRadio', 'on', 0.7)
        --         DisableActions(GetPlayerPed(-1))
        --         TriggerEvent("esx_policejob:startAnim", source)
        --         Busy = true
        -- elseif not IsControlPressed(2, Keys["M"]) and GetLastInputMethod(2) and Busy == true and PlayerData.job.name ~= nil and PlayerData.job.name == "police" then
		-- 		TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 5.0, 'off', 0.7)
		-- 		TriggerServerEvent('esx_policejob:playSoundRadio', 'off', 0.7)
        --         EnableActions(GetPlayerPed(-1))
        --         TriggerEvent("esx_policejob:stopAnim", source)
        --         Busy = false
		-- end
